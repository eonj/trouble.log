...
MySQL 이라는 RDBMS 가 꼭 어떤 필드에 utf8mb3 규격에 맞는 문자열만 담고 있다고 보장할 필요가 있었을까? 이것은 정답이 뭐라고 확신하기 상당히 어려운 문제이다. 이론적으로야 RDBMS 의 일차적 역할은 unique 나 primary key, foreign key 와 같은 관계 대수적 제약을 유지하는 것이고, 그 위에서 join 이나 aggregation 등 관계 대수적 연산을 고속으로 수행할 수 있는지가 RDBMS 의 성능을 판단하는 기준이 된다. 그렇다면 utf8mb3 인코딩 같은 것은 그냥 DBMS 를 사용하는 외부 시스템에서 점검하면 될 이 된다. 그러나 DBMS 는 결국 주기억장치에만 담아 둘 수 없는 규모의 자료를 효율적으로 다루기 위해 사용되기 때문에, 문자열 패턴 매칭 따위의 계산이 DBMS 에 내장되어 있는 것은 일정한 이점을 주며, 이를 위해서는 DBMS 가 레코드를 단순히 직렬화된 바이트 배열로 다루는 것에서 끝나지 않고 탈직렬화 이후의 의미론에 대해서도 DBMS 가 어느 정도 알고 있어야 한다는 결론을 얻는다.
