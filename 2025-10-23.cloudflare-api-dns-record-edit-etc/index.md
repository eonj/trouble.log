...
진단? 이런 문제가 생기는 내재적 원인은, 물론 다양하겠지만, 예컨대 `result.get_dns_records.json` 을 파싱하지 못하는 것일 수 있다. 셸 스크립트의 경우 `jq` 같은 걸 사용했을 법하고 이게 PATH 범위의 명령어에 없었거나 아무튼 예상되지 않은 동작을 했을 수 있다. 물론 이 문제는 ISP DHCP 에서 주는 공인 IP 주소가 변경되고 나서 발견된다. 어느 정도 새너티 체크가 있더라도 모니터링이 없다면 미리 대응할 수 없으며, 트러블슈팅은 사람 몫이다. 규모의 경제에서 그 비용이 희석될 뿐.
